...
<div class="main">
	...
	<table>
		...
		<tr>
			...
			<td>{@value->name}</td>
			<td>{@value->id}</td>
			<td><a href="premission.php?action=update&id={@value->id}">修改</a> | <a href="premission.php?action=delete&id={@value->id}" onclick="return confirm('你确定删除吗') ? true : false">删除</a></td>
		</tr>
		{/foreach}
		{else}
