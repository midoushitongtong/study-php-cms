...
class SearchAction extends Action {
	...
	private function searchTitle() {
		if ($_GET['type'] == 1) {
			if (empty($_GET['inputkeyword'])) Tool::alertBack('搜索内容不得为空');
			...
			Tool::subStr($_object, 'info', 123, 'utf-8');
			Tool::subStr($_object, 'title', 33, 'utf-8');
			if ($_object) {
				foreach ($_object as $_value) {
					if (empty($_value->thumbnail)) $_value->thumbnail = 'images/none.jpg';
					$_value->title = str_replace($this->_model->inputkeyword, '<span style="color: #f60">'.$this->_model->inputkeyword.'</span>', $_value->title);
				}
			}
	...
	private function searchKeyword() {
		if ($_GET['type'] == 2) {
			...
			if ($_object) {
				foreach ($_object as $_value) {
					if (empty($_value->thumbnail)) $_value->thumbnail = 'images/none.jpg';
					$_value->keyword = str_replace($this->_model->inputkeyword, '<span style="color: #f60">'.$this->_model->inputkeyword.'</span>', $_value->keyword);
				}
			}
			$this->_tpl->assign('SearchContent', $_object);
		}
